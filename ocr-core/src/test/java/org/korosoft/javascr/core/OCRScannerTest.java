...
import org.korosoft.javaocr.core.impl.OCRScanner;
...
public class OCRScannerTest {
    ...
    public void testScan() throws Exception {
        ...
        RecognitionSettings settings = new RecognitionSettings(0.5, 0.2, 0.9);
        ...
        List<List<List<RecognizedSymbol>>> scan = scanner.scan(image);
        ...
        RecognizedSymbol letter_t = scan.get(0).get(2).get(3);
        referenceImageStorage.addReferenceImage(letter_t.image, "t", letter_t.baseLine);
        List<List<List<RecognizedSymbol>>> scan2 = scanner.scan(image);
        int t_count = 0;
        for (List<List<RecognizedSymbol>> line : scan2) {
            for (List<RecognizedSymbol> word : line) {
                for (RecognizedSymbol symbol : word) {
                    if (symbol.referenceImage != null && symbol.referenceImage.getSymbol().equals("t")) {
                        t_count++;
                    }
                }
            }
